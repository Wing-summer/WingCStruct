...
void CTypeParser::initialize() {
    ...
    // for (const QString &type : basic_types_) {
    //     type_sizes_[type] = kWordSize_;
    // }

    type_sizes_["void"] = 0;

#define REGISTER_TYPE(t) type_sizes_[#t] = sizeof(t)
    ...
    REGISTER_TYPE(qint8);
    REGISTER_TYPE(quint8);
    REGISTER_TYPE(qint16);
    REGISTER_TYPE(quint16);
    REGISTER_TYPE(qint32);
    REGISTER_TYPE(quint32);
    ...
    REGISTER_TYPE(quint32_le);
    REGISTER_TYPE(qint64_le);
    REGISTER_TYPE(quint64_le);

    REGISTER_TYPE(qint16_be);
    REGISTER_TYPE(quint16_be);
    REGISTER_TYPE(qint32_be);
    REGISTER_TYPE(quint32_be);
    REGISTER_TYPE(qint64_be);
    REGISTER_TYPE(quint64_be);
...
TokenTypes CTypeParser::getTokenType(const QString &token) const {
    if (keywords_.contains(token)) {
        ...
        return kQualifier;
    } else if (type_sizes_.contains(token)) {
...
int CTypeParser::getTypeSize(const QString &data_type) const {
    if (type_sizes_.contains(data_type)) {
        return type_sizes_.value(data_type);
...
void CTypeParser::dumpTypeDefs() const {
    ...
    for (auto it = struct_defs_.constKeyValueBegin();
        ...
        auto type = it->first;
        qout << "\t(size = " << type_sizes_.value(type) << ")\n" << Qt::endl;
    }
    ...
    for (auto itu = union_defs_.constKeyValueBegin();
        ...
        qout << "\t(size = " << type_sizes_.value(itu->first) << ")\n"
             << Qt::endl;
    }
